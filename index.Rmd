...
In some situations it can be interesting to calculate $R^2$. The quantity computed is a [Bayesian generalization](https://github.com/jgabry/bayes_R2/blob/master/bayes_R2.pdf) of the typical $R^2$.

```{r}
bayes_R2(mod_rt2)
```

As with all things Bayesian, we not only get a point estimate but also a distribution.
...
```{r rt loo, warning = F, message = F, cache=T}
